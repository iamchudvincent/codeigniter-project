<?php
session_start();
//Create Verify Code Image 生成验证码图片
...
$cmax = count($list) - 1;
$verifyCode = '';
for ( $i=0; $i < 4; $i++ ){  // VerifyCode Length
...
{
     imagesetpixel($im, rand()%70, rand(), $black);    //加入点状干扰素
     imagesetpixel($im, rand()%70, rand(), $red);
     imagesetpixel($im, rand()%70, rand(), $gray);
